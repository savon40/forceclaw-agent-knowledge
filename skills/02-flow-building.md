...
|-----------|--------------|---------------------|
| `create_records` | `field_values: { "Name": "Acme" }` | ~~`fields`~~, ~~`records: [{ fields: [...] }]`~~ |
| `update_records` | `field_values: { "Status": "Active" }` | ~~`fields`~~, ~~`field_assignments`~~ |
| `decision` | `rules: [{ name, conditions, steps }]` | ~~`outcomes`~~, ~~`branches`~~ |
| `decision` default | `default_steps: [...]`, `default_label: "..."` | ~~`{ "default": true }`~~ inside outcomes |
| `decision` branching | Nest `steps: [...]` inside each rule | ~~`next_step: "Step_Name"`~~ |

The property is **`field_values`** (NOT `fields`). `fields` on a `get_records` step means which fields to return — completely different. Mixing them up causes a crash.

For decisions, use **`rules`** with nested **`steps`** arrays — NOT `outcomes` with `next_step` pointers.
