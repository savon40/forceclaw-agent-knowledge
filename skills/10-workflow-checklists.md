...
## Creating a Custom Object — CRITICAL Access Checklist

A new custom object is invisible to every user (except System Administrator) until you grant **Object Permissions**. Tab visibility and Field-Level Security are necessary but NOT sufficient. In Lightning, if a user's profile has no Read on the object, the tab is hidden from the App Launcher regardless of the tab's `DefaultOn` setting.

When presenting a plan for a new custom object, your plan must ALWAYS include object-level access as a step. The complete post-creation access sequence is:

1. **Object Permissions** — grant Read (and typically Create / Edit / Delete) via `update_profile_object_permissions` for each non-admin profile that needs access. Ask the user which profiles (offer "All Profiles" as an option). Without this, nothing else matters.
...
### MasterDetail parent dependency when granting Object Permissions

When you grant Read on a child object (one that has a MasterDetail to a parent), Salesforce requires the same profile to already have Read on the parent. Error: `Permission Read Child__c depends on permission(s): Read Parent__c`.

Rule: when granting Object Permissions on a child, ALSO grant Read on the MasterDetail parent for the same profile. The `update_profile_object_permissions` tool will attempt to auto-grant parent Read when it detects this error, but you can avoid the round-trip by granting parent permissions first. Example ordering: if you're granting access to `Commission__c` (child of `Commission_Plan__c`), grant permissions on `Commission_Plan__c` first, then `Commission__c`.

### Lookups to the User object

Salesforce does not support Cascade or Restrict delete behavior on lookups to the User object. Consequence: User lookups CANNOT be required (required lookups need Cascade or Restrict). If a user asks for a required User lookup, create it as an optional lookup and either (a) enforce presence with a validation rule, or (b) set the value in a before-save flow. Don't pass `required: true` or `delete_behavior: Cascade/Restrict` when `referenced_object` is `User` — the tool will reject the call immediately with a clear message.
