...
3. **If any picklist values are missing, STOP and tell the user** which values don't exist yet and ask whether they want them created. Do NOT silently add them. Creating a new value is a change the user didn't ask for, and for Opportunity stages it carries forecast semantics (forecast category, probability, won/closed) that you must not guess. Only once the user confirms (and, for Opportunity, supplies the forecast category/probability) do you add them — either with `add_picklist_values` and then `create_business_process`, or `create_business_process` with `auto_add_missing_values: true` in one step. The `auto_add_missing_values` flag is a convenience for AFTER confirmation, never a way to push past a "missing values" error on your own.
4. **Call `create_business_process`** with `object_name`, `process_name`, and the ordered list of `picklist_values` to expose. For Case/Lead you can optionally pass `default_value` (otherwise the first value is the default). **Opportunity does NOT support a default stage** — Salesforce rejects an Opportunity business process that specifies one ("Cannot specify a default on: Opportunity"), so the tool sets no default for Opportunity and any `default_value` you pass is ignored. You can also pass `auto_add_missing_values: true` (see step 3).
