...
## Lightning Web Component (LWC) template rules

LWC HTML templates have STRICT rules — no JavaScript expressions of any kind. Violations cause deploy failures.

### Absolute rules

1. **NO JS expressions in templates** — no ternary, no comparison, no concatenation, no arithmetic
2. **NO expressions in `if:true` / `if:false`** — must be a simple property reference, NEVER a comparison like `{x==='value'}`
...
### CRITICAL: These patterns ALWAYS cause LWC1058 deploy failure
...
```javascript
// In the JS controller — one getter per conditional:
